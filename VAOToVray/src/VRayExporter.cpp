...
void VRayExporter::setFOV(float _fov)
{
  ...
  writePair(m_stream,"fov",_fov*2.0f);
}
...
void VRayExporter::writeVAO(const std::string &_name,ngl::VertexArrayObject *_vao)
{
  ...
  {
    // as we are stored in batches of 3 need to do faces here
    for(auto f=0; f<3; ++f)
    {
      uv.push_back(ngl::Vec3(*(ptr+0),*(ptr+1),0));
      normals.push_back(ngl::Vec3(*(ptr+2),*(ptr+3),*(ptr+4)));
      verts.push_back(ngl::Vec3(*(ptr+5),*(ptr+6),*(ptr+7)));
      ptr+=8;
    }
    vertIndex.push_back(ngl::Vec3(vIndex,vIndex+1,vIndex+2));
    vIndex+=3;
