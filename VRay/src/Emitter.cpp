...
void Emitter::exportFrame()
{
  ...
  t.setPosition(0,0,-2);
  scene.renderView(t.getMatrix());// m_cam->getViewMatrix());
  scene.setImageSize(600,450);
  scene.setFOV(ngl::radians(45.0f));
  scene.setBGColour(ngl::Colour(0.1f,0.1f,0.1f));
  ...
  for(unsigned int i=0; i<m_numParticles; ++i)
  {
    ...
    colours.push_back(ngl::Colour(m_particles[i].m_r,m_particles[i].m_g,m_particles[i].m_b));
  }
  scene.writeGeomParticle("particles",positions,colours,VRayExporter::PointModes::points);
  ngl::Mat4 scale;
 // scale.scale(0.1f,0.1f,0.1f);
  scene.writeNode("ParticleNode","particles","Red_brdf",scale);
